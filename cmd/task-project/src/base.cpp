...
void hex_uuid_to_base64(const char *hex_uuid, char *base64_uuid)
{
    ...
    for (i = 0, j = 0; i < base64_len; i += 4, j += 3)
    {
        ...
        base64_uuid[i] = Base64Chars[(combined >> 18) & 0x3F];
        base64_uuid[i + 1] = Base64Chars[(combined >> 12) & 0x3F];
        base64_uuid[i + 2] = Base64Chars[(combined >> 6) & 0x3F];
        base64_uuid[i + 3] = Base64Chars[combined & 0x3F];
    }
