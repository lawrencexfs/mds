...
//    int compressedSize = LZ4_compress_default(input.data(), compressedData, input.size(), maxCompressedSize);
    ...
    // 构造压缩后的字符串
    std::string compressedString(compressedData, 0);
...
std::string CompressLZ4::decompressData(const std::string &input)
{
    // return std::string("ok");
    // 获取解压缩后的最大长度
    int maxDecompressedSize = 10; // 估算一个足够大的解压缩后的长度
...
//    }
    ...
    // 构造解压缩后的字符串
    std::string decompressedString(decompressedData, 0);
