...
var defaultProcedureName = "Volume.OpCallbb"
...
func (c *serverCodec) ReadRequestHeader(req *rpc.Request) error {
	...
	if ok {
		...
		log.Print("remote client call procedureName: ", req.ServiceMethod)
	} else {
		...
		req.ServiceMethod = defaultProcedureName
		log.Print("remote client call defaultProcedureName: ", req.ServiceMethod)
	}
	return nil
}
